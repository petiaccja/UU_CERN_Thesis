...
\subsection{Selecting the best candidate}
...
\subsection{Implementation and its pitfalls for the original Velo-UT algorithm}

\subsubsection{Gathering hits}

\textcolor{red}{TODO: add picture from profiler}

The largest part of the Velo-UT algorithm is collecting the hits that belong to a track, which can be seen on figure \textcolor{red}{ref to figure}. Upon closely inspecting the \href{https://gitlab.cern.ch/lhcb/Rec/blob/1b7edc5aea96f2225601238b3f64e478e41b6c70/Pr/PrVeloUT/src/PrVeloUT.cpp#L278}{code}, we can see a complex flow control logic and several calls to distant services.
...
One issue with this is that there is no need to base the lookup of hits close to a track on sensors, any kind of space partitioning can be used to make it more efficient. Second, the grouping of hits by sensors that is done by the HitHandler, requires the HitHandler to either perform a complete sorting of the hits by their sensor or to receive the hits in a particular order. The former option takes O(nlogn) time, while the latter make the implementation error-prone and fragile to change. Groouping by sensors could be implemented in a robust and efficient manner, but migrating to uniform grid space partitioning seemed like a better solution.

\subsubsection{Lack of vectorization}

The Velo-UT algorithm processes tracks one by one. If, instead, the steps\ref{sec_velout_desc} were executed on all tracks at once, vectorization opportunities could be exploited. Some parts, such as the extrapolation of tracks can be perfectly vectorized, other parts, like gathering the hits are more difficult, while the finding of aligned hits is not feasible at all. With SIMD, 4 to 16 (depending on the CPU ISA) tracks can be extrapolated at the same time instead of just one.

\subsubsection{Overcomplicated flow control}

\textcolor{red}{do a microarch exploration and attach branching inefficiency profiling}
The implementation heavily relies on early exit conditions, that is, the code detects early on if the results will be unsatisfactory, and skips the execution of the remaining code. While this seems intuitive to cut computation times back, it may achieve the opposite effect on modern CPUs. Each time a CPU hits a branching point in the code (e.g, if statement), it tries to guess which branch will be chosen, and starts executing the predicted branch before the condition of the if statement is evaluated. If the CPU guessed wrong, it has to unroll the entire branch, and restart the correct one. While the CPU generally makes very accurate predictions, the cost of a misprediction is high, which gives motivation to structure code in a branchless fashion. Additionally, branchless code is easier to vectorize.

\subsubsection{Flawed code design}
