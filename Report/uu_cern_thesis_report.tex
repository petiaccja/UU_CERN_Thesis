...
Software is no exception to the 80-20 rule (and its variations), and usually 20\% of the code consumes 80\% of the processor time. It is therefore logical to target optimization efforts on these so-called \textit{hotspots}.

Consquently, the first step of optimization is to identify hotspots, for which developers leverage different \textit{profilers}. LHCb's software has a built-in profiler which breaks down execution time to the level of individual algorithms (see \ref{sec_reco_sw_overview}) that make up the particle path reconstruction. In addition to the built-in profiler, tools such as Intel's VTune Amplifier can show a more detailed view of load distribution that breaks down execution time to the level of C/C++ functions.
...
Figure \ref{fig_algo_usage_choice} shows that the parametrized Kalman fitter takes 47.1\% of the total execution time, followed by the forward tracking which takes roughly a quarter. Based on this and rough profiling with VTune, I decided that the Kalman fitter is the best target for optimization.
